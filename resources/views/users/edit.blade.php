...
@section('content')
<div class="mx-auto max-w-2xl" x-data="userEditForm()">
    <div class="mb-6">
    ...
    <div class="overflow-hidden rounded-xl bg-white shadow-sm ring-1 ring-gray-900/5">
        <div class="px-6 py-5 border-b border-gray-100">
            ...
            <p class="mt-1 text-sm text-gray-500">Update notification recipient details.</p>
        </div>
        <form @submit.prevent="submit" class="px-6 py-6 space-y-5">
            ...
            <div class="rounded-lg bg-gray-50 px-4 py-3 border border-gray-200">
                <dl class="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                    ...
                    <dd class="text-gray-900 font-mono text-xs">{{ $user['uuid'] }}</dd>
                    <dt class="font-medium text-gray-500">Created</dt>
                    <dd class="text-gray-900">{{ !empty($user['created_at']) ? \Carbon\Carbon::parse($user['created_at'])->format('M d, Y H:i') : 'N/A' }}</dd>
                </dl>
            </div>
...
function userEditForm() {
    return {
        ...
        form: {
            name: @js($user['name'] ?? ''),
            email: @js($user['email'] ?? ''),
            phone_e164: @js($user['phone_e164'] ?? ''),
            locale: @js($user['locale'] ?? 'en'),
            timezone: @js($user['timezone'] ?? 'UTC'),
            is_active: @js((bool) ($user['is_active'] ?? true)),
        },
        errors: {},
        ...
        submit() {
            ...
            axios.post('{{ route("users.update", $user["uuid"]) }}', data)
                .then((res) => {
